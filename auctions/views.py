...
def create_listing(request):
    if request.method == "POST":
        ...
        try:
            ...
            new_listing.save()
            messages.success(request, "Success, your listing has been saved!")
            return HttpResponseRedirect(reverse("index"))
...
def create_bid(request, listing_id):
    if request.method == "POST":
        ...
        if bid_form.is_valid():
            bid_form.save()
            messages.success(request, "Success, your bid has been successfully made!")
        else:
...
def filtered_index(request, category):
    return render(request, "auctions/index.html", {
        'Listing': Listing.objects.filter(closed=False, category=category),
        'title': f'Active Listing under "{category}"'
    })
...
def watchlist_page(request):
    assert request.user.is_authenticated
    return render(request, "auctions/index.html", {
        'Listing': request.user.watchlist_items.all(),
